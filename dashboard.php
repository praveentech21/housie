...
<html class="sidebar-light fixed sidebar-left-collapsed">
	...
	<body>
		<section class="body">
            ...
            <?php include "sidebar.php"; ?>
				...
				<section role="main" class="content-body">
					...
					$dpoints_res=mysqli_query($conn, "SELECT sum(points) from winners where pid='$pid';"); 		
          			$dpoints=mysqli_fetch_array($dpoints_res);
									...
									<div class="col-12">
										<section class="card mb-4">
											<div class="card-body bg-primary">
												<div class="widget-summary">
													<div class="widget-summary-col">
														<div class="summary">
															...
															<div class="info"><a href='transactions.php'>
																<strong class="amount" id='points'><i class='fas fa-rupee-sign'></i><?php 
												    if($points[0] > 0)
													{
                                                      echo $points[0]; 
													}
												...
												?> /-</strong></a>
														<div><a href='transactions.php' style='font-size:10px;color:#ffffff;'>View Transactions</a>
														</div>
															</div>
														</div>
													</div>
										...
										<section class="card mb-4">
											<div class="card-body bg-primary">
												<div class="widget-summary">
													<div class="widget-summary-col">
														<div class="summary">
															<h4 class="title">MY WIN POINTS</h4>
															<div class="info"><a href='transactions.php'>
																<strong class="amount" id='points'><?php 
												...
												?></strong></a>
														<div><a href='win_points.php' style='font-size:10px;color:#ffffff;'>View Win Points</a>
														</div>
															</div>
														</div>
													</div>
		...
		<div class="face5" id='f5' style='font-size:20px;padding-top:10px;'>DAILY 2 GAMES<span style='font-size:16px;color:#000000;'><br>From 7:00 PM<br>Onwards</span></div>
							...
							<section class="card mt-4">
								<div class="card-body">
                                        <?php
        								  ...
        								  $game_res=mysqli_query($conn, "SELECT gid, DATE_FORMAT(schedule, '%e %M (%W)') as dt, DATE_FORMAT(schedule, '%h:%i %p') as tm, status, cost FROM games where schedule >= CURDATE() order by schedule limit 10;"); 		

          								  if(mysqli_num_rows($game_res) == 0)
										  {
											echo "<h5 style='color:#0000000;'>No Scheduled Games Today! Stay Tuned...</h5>";  
										  }
                                          else
										  {											  
           								    while($row=mysqli_fetch_array($game_res))
										    {
											  echo "<table class='table table-responsive-md table-striped mb-0'><thead><tr><th>Game Details</th><th>Join Game</th></tr></thead><tbody>";
												
   	        								  $gid=$row['gid'];
											  $sponsor_res=mysqli_query($conn, "SELECT * from sponsors where gid='$gid';");
                 							  if(mysqli_num_rows($sponsor_res) > 0)
											  {
												$srow=mysqli_fetch_array($sponsor_res);
												$gname="<span style='color:green;'>".$srow['gname']."</span>";
											  }
											  else
											  {
                                                $gname="Daily Game";
											  ...
											  if($row[4]>0) {$gtype="PAID GAME";}
											  else {$gtype="";}

   	        								  $status=$row['status'];
											  echo "<tr>";
											  echo "<td><a href='game.php?gid=$gid'><b>#$gid - $gname</b></a><br>";
											  echo "<small><b>$row[1]";
											  echo "<br>$row[2]</b></small></td>";
											 ...
											 else
											 {
                                              ...
                                              if(mysqli_num_rows($sub_res) > 0)
											   {    
        										echo "<td id='gerr$gid'><a href='game.php?gid=$gid'><button type='button' class='mb-1 mt-1 mr-1 btn btn-xs btn-success'>VIEW GAME</button></a><br><small>Joined Game</small></td>";
											   }
											   else 
											   {   
                                                $cst=$row[4];
												$cost = $row[4]>0? "<b><span style=''><i class='fas fa-rupee-sign'></i>".$row[4]." </span></b>" : "<small>FREE GAME</small>";
        										echo "<td id='gerr$gid'><button type='button' class='mb-1 mt-1 mr-1 btn btn-xs btn-primary' onclick='buy_ticket($pid,$gid,$cst);'>JOIN GAME</button><br><small>$gtype $cost</small></td>";
											   }
                                              }   											  
											  echo "</tr>";
										    }											
...
-->						<div class="item"><img class="img-thumbnail" src="img/sponsors/srkrec.jpg" alt=""></div>
		...
		//BUY_TICKET......
		function buy_ticket(pid, gid,cst)
          {
			  
			if(cst > 0)
			{
               if (!confirm("This game will deduct " + cst + " points from your wallet. \nDo you want to continue?")) 
               {  
                 exit; 
               }  
			}		
