		...
		<section class="body">
            ...
            <?php include "sidebar.php"; ?>
				...
				<section role="main" class="content-body">
					...
					$points_res=mysqli_query($conn, "SELECT sum(points) from winners where pid='$pid';"); 		
          			$points=mysqli_fetch_array($points_res);
					
					$dpoints_res=mysqli_query($conn, "SELECT sum(points) from points where pid='$pid';"); 		
									...
									<div class="col-12">
										<section class="card mb-4">
											<div class="card-body bg-primary">
												...
												?> Points</strong></a>
														<div><a href='transactions.php' style='font-size:10px;color:#ffffff;'><i class='fas fa-rupee-sign'></i>View Transactions</a>
														</div>
															</div>
														</div>
													</div>
								...
								<div class="card-body">
									<table class="table table-responsive-md table-striped mb-0">
                                        ...
                                        <?php
        								  ...
        								  $game_res=mysqli_query($conn, "SELECT gid, DATE_FORMAT(schedule, '%W %e %M') as dt, DATE_FORMAT(schedule, '%h:%i %p') as tm, status, cost FROM games where schedule >= CURDATE() order by schedule limit 10;"); 		
          								  ...
          								  while($row=mysqli_fetch_array($game_res))
										    {
											 ...
											 else
											 {
                                              ...
                                              if(mysqli_num_rows($sub_res) > 0)
											   ...
											   {   
                                                $cst=$row[4];
												$cost = $row[4]>0? "<b><span style='color:yellow;'>".$row[4]." POINTS</span></b>" : "<span style='color:yellow;'>FREE GAME</span>";
        										echo "<td id='gerr$gid'><button type='button' class='mb-1 mt-1 mr-1 btn btn-xs btn-primary' onclick='buy_ticket($pid,$gid,$cst);'>JOIN GAME ($cost)</button></td>";
											   }
                                              }   											  
											  echo "</tr>";
										    }											
