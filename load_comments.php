...
//	  $comments=mysqli_query($conn, "SELECT msg_id, p.pid, player_name, place, msg, DATE_FORMAT(timestamp, '%l:%i %p') FROM comments c , players p where p.pid=c.pid and gid=$gid and TIMESTAMPDIFF(SECOND, timestamp, now()) <= 5 order by timestamp desc;"); 		
