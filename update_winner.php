	...
	$winner_res=mysqli_query($conn, "SELECT * FROM winners where gid=$gid order by win_type;"); 		
  	while($win=mysqli_fetch_array($winner_res))
	   {   
		...
		if($pid=="") { 
		   $table=$table."<span style='color:green;'>Scratch Card</span>";
		   $winner_details="Not Yet Claimed";
		}
		else 
		{
          ...
          $table=$table."</span>";
  		  ...
  		  $person_details=mysqli_query($conn, "SELECT * FROM players where pid=$pid;"); 		
